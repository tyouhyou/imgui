// All codes here in this file, except draw_with_painter(), are copied from 
...
static void draw_with_painter(int fd);
...
static void draw_with_painter(int fd)
{
    ...
    auto data = (uint32_t*)painter.PaintShapes();
		...
		for (iter = modeset_list; iter; iter = iter->next) {
			...
			for (j = 0; j < height; ++j) {
				for (k = 0; k < width; ++k) {
					off = buf->stride * j + k * 4;
					dataOff = width * j + k;
					*(uint32_t*)&buf->map[off] = data[dataOff];
				}
			}
			...
			ret = drmModeSetCrtc(fd, iter->crtc, buf->fb, 0, 0,
					     &iter->conn, 1, &iter->mode);
			if (ret)
				IE << "cannot flip CRTC for connector " << iter->conn << " (" << errno << ")";
			else
				iter->front_buf ^= 1;
		}
